        ...
        for pit in self:
            ...
            try:
                ...
                if "direction" in result_dict and result_dict["direction"] == "user_to_app":
                    ...
                    if (result_dict['status']['cancelled'] or result_dict['status']['user_cancelled']) and pit.action not in ["cancelled", "complete"] and pit.action_type == "receive":
...
class admin_apps(models.Model):
    ...
    amount_latinchain_token = fields.Float('Amount LatinChain Token Pay', digits=(50,7), default=10, store=True, groups="website_pinetwork_odoo.group_pi_admin,base.group_system")
    ...
    def _update_amount_price(self):
        for i in self:
            ...
            re = requests.post(url,json=obj,headers={ 'x-api-key': admin_app[0].amount_price_apikey }, timeout=(6.05, 6))
    ...
    def _compute_daily(self):
        for i in self:
            ...
            pi_user_list_ids = self.env["pi.users"].sudo()._search([], limit=50, order="points desc,unblocked_datetime desc,points_datetime asc,id asc")
                ...
                if "direction" in result_dict and result_dict["direction"] == "user_to_app":
                    if "paymentType" in result_dict["metadata"] and result_dict["metadata"]["paymentType"] == "tip":
                        if result_dict["status"]["transaction_verified"] and result_dict["status"]["developer_approved"] and result_dict["status"]["developer_completed"]:
                            ...
                            self.env.cr.execute("SELECT id FROM pi_users WHERE id = %s FOR UPDATE", [pi_user[0].id])
...
class pi_users(models.Model):
    ...
    iq_name = fields.Char('IQ Name', default='', groups="website_pinetwork_odoo.group_pi_admin,base.group_system")
    ...
    def _compute_avatar_user_url(self):
        for i in self:
            i.avatar_user_url = "/website_pinetwork_games_odoo/static/src/img/avatars_users/" + i.avatar_user + ".jpeg?v=1.106"
