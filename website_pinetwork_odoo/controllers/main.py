...
class PiNetworkBaseController(http.Controller):
    ...
    def api_external_docs(self, **kw):
        return http.request.redirect('https://github.com/rockcesar/LatinChainDevelops/tree/master/docs')
    ...
    @http.route(['/google-ai-course/', '/ai-master-course/'], type='http', auth="public", website=True, methods=['GET'], csrf=False)
    ...
    @http.route(['/english-book/', '/english-book/paperback', '/english-book/ebook', '/english-book/hardcover'], type='http', auth="public", website=True, methods=['GET'], csrf=False)
    def english_book(self, **kw):
        ...
        elif accessed_path in ['/english-book/hardcover']:
            return http.request.redirect('https://www.amazon.com/dp/B0HK9QJ7FS')
    ...
    @http.route(['/history-ven-book/', '/history-ven-book/paperback', '/history-ven-book/ebook', '/history-ven-book/hardcover'], type='http', auth="public", website=True, methods=['GET'], csrf=False)
    def history_ven_book(self, **kw):
        accessed_path = http.request.httprequest.path
        if accessed_path in ['/history-ven-book/', '/history-ven-book/paperback']:
            return http.request.redirect('https://www.amazon.com/dp/B0HGGXFLJ1')
        elif accessed_path in ['/history-ven-book/ebook']:
            return http.request.redirect('https://www.amazon.com/dp/B0HGH1V2JT')
    ...
    def bstocks_book(self, **kw):
        ...
        if accessed_path in ['/bstocks-book/', '/bstocks-book/paperback']:
            return http.request.redirect('https://www.amazon.com/dp/B0HJ8Q649H')
        elif accessed_path in ['/bstocks-book/ebook']:
            return http.request.redirect('https://www.amazon.com/dp/B0GX2YC5M7')
        elif accessed_path in ['/bstocks-book/hardcover']:
            return http.request.redirect('https://www.amazon.com/dp/B0HKPW2XXS')
    
    @http.route(['/bitcoin-book/', '/bitcoin-book/paperback', '/bitcoin-book/ebook', '/bitcoin-book/hardcover'], type='http', auth="public", website=True, methods=['GET'], csrf=False)
    def bitcoin_book(self, **kw):
        accessed_path = http.request.httprequest.path
        if accessed_path in ['/bitcoin-book/', '/bitcoin-book/paperback']:
            return http.request.redirect('https://www.amazon.com/dp/B0HJ99J7RX')
        elif accessed_path in ['/bitcoin-book/ebook']:
    ...
    @http.route('/join-mexc/', type='http', auth="public", website=True, methods=['GET'], csrf=False)
    def join_mexc(self, **kw):
        return http.request.redirect('https://www.mexc.com/acquisition/custom-sign-up?shareCode=mexc-latinchain')
    
    @http.route('/trade-pi-on-mexc/', type='http', auth="public", website=True, methods=['GET'], csrf=False)
    def trade_pi_on_mexc(self, **kw):
        return http.request.redirect('https://www.mexc.com/exchange/Pi_USDT?shareCode=mexc-latinchain')
    ...
    def get_user(self, **kw):
        ...
        return json.dumps({'result': True, 'pi_user_id': pi_users_list[0].pi_user_id, 'pi_user_code': pi_users_list[0].pi_user_code,
                            ...
                            'user_tips': pi_users_list[0].user_tips
                            })
    ...
    def set_latin_points_ai(self, **kw):
        ...
        if len(pi_users_list) == 0:
            ...
            if not (pi_users_list[0].unblocked and admin_app_list[0].mainnet in ['Mainnet OFF', 'Mainnet ON']):
                return json.dumps({'result': False})
        ...
        return json.dumps({'result': True, 'points_latin': admin_app_list[0].points_latin_amount, 'x2_game': pi_users_list[0].x2_game})
                ...
                if 'app_client' in kw:
                    ...
                    if kw['app_client'] == "auth_platform":
                        if points > 0 and points <= 40:
                            values.update({'points_latin': pi_users_list[0].points_latin + points})
    ...
    def get_transactions_data(self, source, **kw):
        ...
        for i in pi_transactions_list:
            direction = ""
            if i.token_type == "pinetwork":
                token_type = "Pi"
            elif i.token_type == "latinchain":
                token_type = "LatinChain"
            else:
                token_type = "Pi"
            ...
            if source == "radioforus":
                data.append({'memo': i.memo + direction + " (" + str(i.create_date) + " UTC)", 'amount': str('{:,}'.format(i.amount)) + " " + token_type, 'txid': " <button class='btn btn-link' onclick='$.colorbox({fixed: true, href:\"" + i.txid_url + "\", iframe:true, width: \"100%\", height: \"90%\", maxWidth: \"100%\", maxHeight: \"90%\", top: \"0%\", bottom: \"10%\", onOpen: function(){$(\"body\").css(\"overflow\", \"hidden\");}, onClosed: function(){$(\"body\").css(\"overflow\", \"auto\");}});'>" + i.txid[:4] + "..." + i.txid[-4:] + "</button><button class='btn btn-primary' onclick='copyToClipboard(\"" + i.txid_url + "\");'>Copy</button>"})
            else:
                data.append({'memo': i.memo + direction + " (" + str(i.create_date) + " UTC)", 'amount': str('{:,}'.format(i.amount)) + " " + token_type, 'txid': " <button class='btn btn-link' onclick='$.colorbox({fixed: true, href:\"" + i.txid_url + "\", iframe:true, width: \"100%\", height: \"100%\", maxWidth: \"100%\", maxHeight: \"100%\"});'>" + i.txid[:4] + "..." + i.txid[-4:] + "</button><button class='btn btn-primary' onclick='copyToClipboard(\"" + i.txid_url + "\");'>Copy</button>"})
