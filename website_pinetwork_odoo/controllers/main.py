...
class PiNetworkBaseController(http.Controller):
    ...
    def get_transactions_data(self, **kw):
        ...
        for i in pi_transactions_list:
            ...
            data.append({'memo': i.memo + direction + " (" + str(i.create_date) + " UTC)", 'amount': str(i.amount) + " Pi", 'txid': " <button class='btn btn-link' onclick='$.colorbox({href:\"" + i.txid_url + "\", iframe:true, width: \"100%\", height: \"80%\", maxWidth: \"100%\", maxHeight: \"80%\", onOpen: function(){$(\"body\").css(\"overflow\", \"hidden\");}, onClosed: function(){$(\"body\").css(\"overflow\", \"auto\");}});'>" + i.txid[:4] + "..." + i.txid[-4:] + "</button><button class='btn btn-primary' onclick='copyToClipboard(\"" + i.txid_url + "\");'>Copy</button>"})
