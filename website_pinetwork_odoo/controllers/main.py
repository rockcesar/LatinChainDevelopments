...
class PiNetworkBaseController(http.Controller):
    ...
    def get_credits_data(self, **kw):
        ...
        pi_users_list = request.env["pi.users"].sudo().search([('donator', '=', True), ('pi_user_code', 'ilike', '%' + searchValue + '%')], order="unblocked desc, paid_in_all_donations desc", limit=int(rowperpage), offset=int(row))
