...
class PiNetworkBaseController(http.Controller):
    ...
    def get_user(self, **kw):
        ...
        resting_time = str(timedelta(seconds=(datetime.now()-(datetime.now() - timedelta(seconds=pi_ad_seconds))).total_seconds()))
    ...
    def get_transactions_data(self, source, **kw):
        ...
        for i in pi_transactions_list:
            ...
            if source == "radioforus":
                data.append({'memo': i.memo + direction + " (" + str(i.create_date) + " UTC)", 'amount': str('{:,}'.format(i.amount)) + " Pi", 'txid': " <button class='btn btn-link' onclick='$.colorbox({fixed: true, href:\"" + i.txid_url + "\", iframe:true, width: \"100%\", height: \"90%\", maxWidth: \"100%\", maxHeight: \"90%\", top: \"0%\", bottom: \"10%\", onOpen: function(){$(\"body\").css(\"overflow\", \"hidden\");}, onClosed: function(){$(\"body\").css(\"overflow\", \"auto\");}});'>" + i.txid[:4] + "..." + i.txid[-4:] + "</button><button class='btn btn-primary' onclick='copyToClipboard(\"" + i.txid_url + "\");'>Copy</button>"})
            else:
                data.append({'memo': i.memo + direction + " (" + str(i.create_date) + " UTC)", 'amount': str('{:,}'.format(i.amount)) + " Pi", 'txid': " <button class='btn btn-link' onclick='$.colorbox({fixed: true, href:\"" + i.txid_url + "\", iframe:true, width: \"100%\", height: \"100%\", maxWidth: \"100%\", maxHeight: \"100%\"});'>" + i.txid[:4] + "..." + i.txid[-4:] + "</button><button class='btn btn-primary' onclick='copyToClipboard(\"" + i.txid_url + "\");'>Copy</button>"})
