            ...
            if(data.result)
            {
                ...
                else
                {
                    ...
                    $("#test_app").click(function(){
                        ...
                        {
                            alert("Max rewarded ads number reached. " + pi_ad_max + " rewarded ads each " + show_pi_ad_user_time + "h. Resting time: " + resting_time + "h.");
                        }
                    });
                }
            }
            ...
            if(pi_user_id != "" && pi_user_code != "" && showAdResponse.adId)
            {
                ...
                $.post( "/set-latin-points", data).done(function(data) {
                    ...
                    if(data.result && data.points_latin > 0)
                    {
                        ...
                        if(data.x2_game)
                            text += '\nYou have now x2 points in your next game (Chess, Sudoku and Snake, only for PREMIUM users).';
    ...
    async function transfer() {
        try {
            ...
            const payment = Pi.createPayment({
              ...
              memo: "Unlock LatinChain PREMIUM (Sudoku)", // e.g: "Digital kitten #1234",
