...
    const degRadBtn = document.getElementById('deg-rad-btn'); // Added DEG/RAD button reference
    
    // Memory buttons
...
        if (operation != null) {
            previousOperandElement.innerText = `${formatDisplayNumber(previousOperand)} ${getOperationSymbol(operation)}`;
        } else {
            previousOperandElement.innerText = formatDisplayNumber(previousOperand);
        }
        
        // Update memory indicator
        if (memoryStored) {
            memoryIndicatorElement.innerText = `M: ${formatDisplayNumber(memory.toString())}`;
...
        if (degRadBtn) {
            degRadBtn.textContent = isRadiansMode ? 'RAD' : 'DEG';
        }
    }
    
    // Format numbers for display to prevent overflow
...
        }
        
        // Update display for all functions except those that wait for second operand
        if (func !== 'power' && func !== 'mod' && func !== 'nthroot' && func !== 'deg' && func !== 'rad') {
            updateDisplay();
        }
    }
    
...
            updateDisplay(); // Always update display after scientific functions
        });
    });
    
...
    // Event listener for the DEG/RAD toggle button
    if (degRadBtn) {
        degRadBtn.addEventListener('click', () => {
            isRadiansMode = !isRadiansMode; // Toggle the state
            updateDisplay(); // Update the button text and display
        });
    }
    // === END OF ADDED LOGIC ===
    
